...
use worker::{console_error, Context, Date, Env, Fetch, HttpRequest};
...
pub async fn proxy_ws(
    ...
    if !(is_upgrade && is_websocket) {
        return Err((StatusCode::BAD_REQUEST, "Request not supported".to_string()));
    }
    ...
    let pair = WebSocketPair::new().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let server = pair.server;
    let client = pair.client;
    server.accept().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let worker_resp = worker::Response::from_websocket(client).map_err(|e| {
...
pub async fn proxy(
    ...
    let body_stream: LocalBoxStream<'static, std::result::Result<Vec<u8>, worker::Error>> =
