...
mod tests {
    ...
    fn fake_proxy() -> Proxy {
        // 仅用于解析命令测试;algo/ip 的 set_* 在未启动时也可调用(热切换状态)
        let id = TMP_ID.fetch_add(1, Ordering::Relaxed);
        let dir = std::env::temp_dir().join(format!(
            "client_cli_test_ca_{}_{id}",
            std::process::id()
        ));
        let secret = crate::ca::key_secret_for(&dir).unwrap();
        let mut s = ProxySettings::defaults();
        s.domain = "free-proxy.test.workers.dev".into();
        s.auth_key = "test-key".into();
        build_proxy(&s, dir, secret).unwrap()
    }

    #[tokio::test]
    async fn parse_unknown_and_stop() {
        let p = fake_proxy();
        assert!(!handle_stdin("hello", &p).await);
        assert!(handle_stdin("stop", &p).await);
        assert!(handle_stdin("exit", &p).await);
        assert!(!handle_stdin("", &p).await);
    }

    #[tokio::test]
    async fn parse_algo_hot_switch() {
        let p = fake_proxy();
        assert!(!handle_stdin("algo zstd aes128gcm", &p).await);
        let algo = p.algo();
        assert_eq!(algo.compressor.name(), "zstd");
        assert_eq!(algo.aead.name(), "aes128gcm");
    }

    #[tokio::test]
    async fn parse_algo_bad_input_keeps_state() {
        let p = fake_proxy();
        assert!(!handle_stdin("algo 不存在算法 aes128gcm", &p).await);
        assert_eq!(p.algo().compressor.name(), "zstd");
    }

    #[tokio::test]
    async fn parse_ip_hot_switch() {
        let p = fake_proxy();
        assert!(!handle_stdin("ip 104.16.39.227", &p).await);
        assert_eq!(p.ip().as_deref(), Some("104.16.39.227"));
        assert!(!handle_stdin("ip off", &p).await);
        assert_eq!(p.ip(), None);
    }
}